...
int main(void){
	...
	if (ret == 0){ //if siglongjmp didnt occur ...
		//set handler and do sigaction SIGINT with act_sig1
		act_sig1.sa_handler = ssu_signal_handler1; 
		sigaction(SIGINT, &act_sig1, NULL);
	}
	else if (ret == 3){ //if siglongjmp occured ...
		printf("-----------------------------\n");
	}

	printf("Starting\n"); 
	sigsetjmp(jmp_buf2, 2); //set jmp_buf2
	//set handler and do sigaction SIGUSR1 with act_sig2
